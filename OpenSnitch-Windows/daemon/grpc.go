...
// startGRPCServer starts and runs a gRPC server for the UI service, preferring a Windows named pipe and falling back to TCP.
// 
// The function attempts to listen on the named pipe `\\.\pipe\opensnitch` and, if that fails, listens on `127.0.0.1:50051` (TCP fallback).
// It registers a GRPCServer as the UIServer implementation and blocks serving on the chosen listener until the provided context is canceled,
